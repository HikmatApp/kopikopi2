...
@section('page-subtitle', 'Informasi akun ' . (auth()->user()->isAdmin() ? 'administrator' : 'mitra') . ' KopiKopi')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
        ...
        <span class="inline-block mt-3 text-xs font-semibold px-3 py-1 rounded-full {{ auth()->user()->isAdmin() ? 'bg-gray-900 text-white' : 'bg-orange-100 text-orange-700' }}">
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
    ...
    <form action="{{ route('pengaturan.profil') }}" method="POST" class="p-6 space-y-4">
        @csrf
        @method('PUT')
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <dl class="space-y-3 text-sm">
            <div><dt class="text-gray-400">Nama Pengguna</dt><dd class="font-semibold text-gray-800">{{ auth()->user()->name }}</dd></div>
            <div><dt class="text-gray-400">Email</dt><dd class="font-semibold text-gray-800">{{ auth()->user()->email }}</dd></div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <form action="{{ route('pengaturan.password') }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
    ...
    <form action="{{ route('admin.pengaturan.aplikasi') }}" method="POST" class="p-6 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="kontak" value="{{ old('kontak', $pengaturan->kontak) }}"
                    class="w-full border border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-orange-400 outline-none">
            </div>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Ambang Batas Stok Minimum Default</label>
            <input type="number" name="stok_minimum_default" min="0" value="{{ old('stok_minimum_default', $pengaturan->stok_minimum_default) }}" required
                class="w-full border border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-orange-400 outline-none">
        </div>
        <div class="flex justify-end">
